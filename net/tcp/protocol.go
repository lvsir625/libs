...
func (p *MsgParser) Read(conn net.Conn) ([]byte, error) {
	...
	if msgLen > p.maxRecvMsgLen {
		return nil, errors.New(fmt.Sprintf("read message too long[ %d > %d]", msgLen, p.maxRecvMsgLen))
	}
...
func (p *MsgParser) Write(conn net.Conn, args ...[]byte) error {
	...
	if msgLen > p.maxSendMsgLen {
		return errors.New(fmt.Sprintf("write message too long[ %d > %d]", msgLen, p.maxSendMsgLen))
	}
